...
void ISAM<KeyType>::build(std::vector<std::pair<KeyType, POS_TYPE>> &data) {
    ...
    if (data.size() < max_records(ISAM_LEVELS))
        throw std::runtime_error("It's necesary have more than " + std::to_string(max_records(ISAM_LEVELS)) +
...
template<typename KeyType>
POS_TYPE ISAM<KeyType>::_insert(POS_TYPE node_pos, std::pair<KeyType, POS_TYPE> record) {
    // Read the node
    idx_file.seekg(node_pos, std::ios::beg);
    IndexPage<KeyType> node;
    node.read(idx_file);
...
template<typename KeyType>
void ISAM<KeyType>::_range_search(POS_TYPE node_pos, KeyType beg, KeyType end, std::vector<POS_TYPE> &result) {
    // Read the node
    idx_file.seekg(node_pos, std::ios::beg);
    IndexPage<KeyType> node;
    node.read(idx_file);
...
template<typename KeyType>
POS_TYPE ISAM<KeyType>::remove(KeyType key) {
    if (root == -1)
        throw std::runtime_error("No indexed data in ISAM, please build before");

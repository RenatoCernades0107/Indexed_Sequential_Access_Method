 ...
 *              |-----------------keys-----------------|
 * |------------------children---------------|   |-----next-----|
 * |-----count-----| BufferSize = sizeof(KeyType*) + (sizeof(KeyType) * N) +
 * sizeof(KeyType*) + sizeof(POS_TYPE) * (N+1) + sizeof(POS_TYPE) +
 * sizeof(POS_TYPE) BufferSize = sizeof(KeyType*) + (sizeof(KeyType) * N) +
 * sizeof(KeyType*) + (sizeof(POS_TYPE) * N) + sizeof(POS_TYPE) +
 * sizeof(POS_TYPE) + sizeof(POS_TYPE) BufferSize = N * (sizeof(KeyType) +
 * sizeof(POS_TYPE)) + (3 * sizeof(POS_TYPE)) + (2 * sizeof(KeyType*)) N =
 * (BufferSize - 3 * sizeof(POS_TYPE) - 2 * sizeof(KeyType*)) / (sizeof(KeyType)
 * + sizeof(POS_TYPE))
 *
 */

template <typename KeyType> int64_t DataPage<KeyType>::getCapacity() {
  auto a = std::floor((buffer::get_buffer_size() - 3 * sizeof(POS_TYPE) -
                       2 * sizeof(KeyType *)) /
                      (sizeof(KeyType) + sizeof(POS_TYPE)));
  return 3;
}

template <typename KeyType> KeyType *DataPage<KeyType>::getKeys() const {
  return keys;
}
...
void DataPage<KeyType>::setKey(POS_TYPE key, int64_t pos) {
  try {
    ...
    throw(std::runtime_error("Cannot read record: " + std::to_string(key) +
...
template <typename KeyType> POS_TYPE *DataPage<KeyType>::getRecords() const {
  return records_pos;
}

template <typename KeyType>
void DataPage<KeyType>::setRecord(std::pair<KeyType, POS_TYPE> record,
                                  int64_t pos) {
  try {
...
template <typename KeyType> void DataPage<KeyType>::setNext(POS_TYPE next) {
  DataPage::next = next;
}

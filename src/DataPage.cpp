...
template <typename KeyType> DataPage<KeyType>::DataPage() {
  ...
  records_pos = new POS_TYPE[static_cast<unsigned long>(capacity)];
  next = -1;
  count = 0;
}
...
template <typename KeyType> int64_t DataPage<KeyType>::size_of() {
  ...
  size += sizeof(KeyType) * static_cast<unsigned long>(capacity);
  size += sizeof(POS_TYPE) * (static_cast<unsigned long>(capacity));
  return size;
}
...
//    auto a = std::floor((buffer::get_buffer_size() - 2 * sizeof(POS_TYPE) - 2 * sizeof(KeyType *)) /
...
template <typename KeyType>
void DataPage<KeyType>::setKey(KeyType key, int64_t pos) {
  try {
    keys[pos] = key;

...
template <typename KeyType> void IndexPage<KeyType>::setIsLeaf(bool isLeaf) {
  this->isLeaf = isLeaf;
}
...
template <typename KeyType> int64_t IndexPage<KeyType>::size_of() {
  int64_t capacity = this->getCapacity();
  return sizeof(KeyType) * capacity + sizeof(POS_TYPE) * (capacity + 1) +
         sizeof(bool);
}
...
template <typename KeyType> int64_t IndexPage<KeyType>::getCapacity() const {
